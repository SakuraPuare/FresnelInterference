...
FringeAnalysisWidget::FringeAnalysisWidget(QWidget *parent)
    : QWidget(parent), m_pixelSize_um(3.45) // Default value
      ,
  ...
  m_previewUpdateTimer->setInterval(
      100); // 100ms debounce time for smoother tuning

  connect(m_startButton, &QPushButton::toggled, this,
          &FringeAnalysisWidget::toggleFreezeMode);

  // Connect ALL parameter changes to schedule a re-analysis
  connect(m_brightnessSlider, &QSlider::valueChanged, this,
          &FringeAnalysisWidget::scheduleReanalysis);
  connect(m_contrastSlider, &QSlider::valueChanged, this,
          &FringeAnalysisWidget::scheduleReanalysis);
  connect(m_gammaSlider, &QSlider::valueChanged, this,
          &FringeAnalysisWidget::scheduleReanalysis);
  connect(m_peakThreshSlider, &QSlider::valueChanged, this,
          &FringeAnalysisWidget::scheduleReanalysis);
  connect(m_minPeakDistSlider, &QSlider::valueChanged, this,
          &FringeAnalysisWidget::scheduleReanalysis);
  connect(m_detectValleysCheck, &QCheckBox::toggled, this,
          &FringeAnalysisWidget::scheduleReanalysis);
...
FringeAnalysisWidget::~FringeAnalysisWidget() { delete m_previewUpdateTimer; }
...
void FringeAnalysisWidget::setupUI() {
  ...
  m_processedImageLabel->setStyleSheet(
      "QLabel { background-color: #2b2b2b; color: white; }");
  ...
  m_resultsTable->setHorizontalHeaderLabels(
      {"条纹序号", "位置 (px)", "与上一条纹间距 (px)"});
  m_resultsTable->setMinimumHeight(150);
  m_resultsTable->horizontalHeader()->setStretchLastSection(true);
  m_resultsTable->setStyleSheet("QTableWidget { background-color: #2b2b2b; "
                                "color: white; gridline-color: #555555; }");
  ...
  m_peakThreshSlider->setRange(1, 254);
  m_peakThreshSlider->setValue(50);
  paramsLayout->addRow("亮/暗检测阈值:", m_peakThreshSlider);

  m_minPeakDistSlider = new QSlider(Qt::Horizontal);
  m_minPeakDistSlider->setRange(1, 100);
  m_minPeakDistSlider->setValue(10);
  ...
  m_pixelSizeLabel =
      new QLabel(QString("%1 μm").arg(m_pixelSize_um, 0, 'f', 2));
  ...
  m_contrastSlider->setRange(1, 200);
...
void FringeAnalysisWidget::resizeEvent(QResizeEvent *event) {
  ...
  m_originalImageLabel->setPixmap(
      originalPixmap.scaled(m_originalImageLabel->size(), Qt::KeepAspectRatio,
                            Qt::SmoothTransformation));

  if (!m_currentPixmap.isNull()) {
    m_processedImageLabel->setPixmap(
        m_currentPixmap.scaled(m_processedImageLabel->size(),
...
void FringeAnalysisWidget::updatePreviewImage() {
  ...
  m_currentPixmap = QtCvUtils::preprocessToQPixmap(
      m_originalFrame, m_brightnessSlider->value(), m_contrastSlider->value(),
      m_gammaSlider->value());
  if (!m_currentPixmap.isNull()) {
    m_processedImageLabel->setPixmap(
        m_currentPixmap.scaled(m_processedImageLabel->size(),
...
void FringeAnalysisWidget::performAnalysis() {
  ...
  QtCvUtils::applyPreprocessing(
      m_frozenFrame, processedFrame, m_brightnessSlider->value(),
      m_contrastSlider->value(), m_gammaSlider->value());
  ...
  cv::warpAffine(grayFrame, rotatedFrame, rot, grayFrame.size(),
  ...
  QtCvUtils::findPeaks(proj_vec, m_peakThreshSlider->value(),
                       m_minPeakDistSlider->value(), peak_indices);
  ...
  if (peak_indices.size() > 1) {
    ...
    results << QString("平均间距: %1 px  (%2 μm)")
                   .arg(avg_spacing_px, 0, 'f', 2)
                   .arg(avg_spacing_um, 0, 'f', 2);

    // Populate table
    m_resultsTable->setRowCount(peak_indices.size());
    for (size_t i = 0; i < peak_indices.size(); ++i) {
      m_resultsTable->setItem(i, 0,
                              new QTableWidgetItem(QString::number(i + 1)));
      m_resultsTable->setItem(
          i, 1, new QTableWidgetItem(QString::number(peak_indices[i])));
      if (i > 0) {
        m_resultsTable->setItem(
            i, 2, new QTableWidgetItem(QString::number(spacings[i - 1])));
      } else {
  ...
  drawResult(displayProcessed, 0, peak_indices,
             projection); // In rotated, angle is 0
...
void FringeAnalysisWidget::drawResult(cv::Mat &displayImage, double angle,
  ...
  for (int peak_x : peak_indices) {
    ...
    if (transformed_points.size() == 2) {
      cv::line(displayImage, transformed_points[0], transformed_points[1],
               color, thickness);
    }
  }
}
...
/**
 * @brief Estimate the rotation angle to make fringes vertical using projection
 * variance search.
...
double FringeAnalysisWidget::estimateFringeRotation(const cv::Mat &gray) {
  ...
  if (gray.depth() != CV_8U) {
    ...
    gray.convertTo(gray8, CV_8U, 255.0 / (maxVal - minVal + 1e-5),
  ...
  for (double ang = -45.0; ang <= 45.0; ang += 1.0) {
    ...
    cv::Mat rotImg;
    cv::warpAffine(gray8, rotImg, rotMat, gray8.size(), cv::INTER_LINEAR,
                   cv::BORDER_REPLICATE);
